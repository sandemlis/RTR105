...
void main(){
 ...
 printf("Mazākā vērtība no šiem simboliem ir %d. simbolam \"%c\".\n",min[1]+1,min[0]);
 printf("Lielākā vērtība no šiem simboliem ir %d. simbolam \"%c\".\n",max[1]+1,max[0]);
 ...
 printf("Simbolu rindas mediāna ir %d. simbols \"%c\" \n",(i+1)/2,string[i/2]);
 char n=0,m=0;
 char mod[i][2];
 mod[0][0]=string[0];
 for(char l=0;l<=i;l++){
 ...
 for(n=0;n<m;n++){
  if(biezums==mod[n][1])printf("Simbolu rindas moda ir simbols \"%c\", kura atkārtojās %d reižu.\n",mod[n][0],biezums);
 }
 for(m=0;m<2;m++){
  for(n=0;n<i;n++){
   if(m==0)printf("%c\t",string[n]);
   if(m==1)printf("%d\t",string[n]);
  }
  printf("\n");
 }
}
